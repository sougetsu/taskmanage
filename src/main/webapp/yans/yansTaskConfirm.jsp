...
<script type="text/javascript">
	$(function() {
		...
		$("#yans_taskConfirm_attach").uploadify({
			...
			queueID:"yans_taskConfirm_attachs",//上传队列说存储的位置
		...
		function fileComplete(event, ID, fileObj, response, data) {
			...
			if(checkOp(rel)) {
				...
				$("#yans_taskConfirm_alreadyAttachs").append(node);
			} else {
				alert(rel.err);
			}
		}
	...
	function confirmOKSubmit(){
		var submitForm = $('#yans_taskOrder_confirmForm');
		if(submitForm.form('validate')){
    		...
    		$.ajax({
                type: "POST",
                url:'${pageContext.request.contextPath}/yansTaskManage/confirmOK',
	...
	function confirmNGSubmit(){
		...
		if(submitForm.form('validate')){
    		...
    		$.ajax({
                type: "POST",
                url:'${pageContext.request.contextPath}/yansTaskManage/confirmNG',
                data:submitForm.serialize(),
                dataType : 'json',
				success : function(result) {
					parent.$.messager.progress('close');
	...
	function setAttachRow(value){
		var taskConfirm_attachSelect = document.getElementById("yans_taskConfirm_attachSelect");
		var taskConfirm_attachSubmit = document.getElementById("yans_taskConfirm_attachSubmit");
	...
	function setErsaiRow(value){
		var ersaiNumSelect = document.getElementById("yans_taskConfirm_ersaiNum");
...
<div data-options="region:'center',border:false">
	<div align="center">
		<form id="yans_taskOrder_confirmForm"method="post">
			...
			<table class="tasktableForm" width="95%">
				...
				<c:choose>
				   	<c:when test="${taskOrder.attachmentFlag==1}">
						<tr id="yans_taskConfirm_attachSelect">
							...
							<td style="text-align:left" colspan=7>
								<div id="yans_taskConfirm_attachs"></div>
								<input type="file" id="yans_taskConfirm_attach" name="attach"/>
								<br/>
								<a href="javascript:$('#yans_taskConfirm_attach').uploadifyUpload()">上传文件</a>
							</td>
						</tr>
						<tr id="yans_taskConfirm_attachSubmit">
							<th width="150px" >已上传附件：</th>
							<td style="text-align:left" colspan=7>
								<div id="yans_taskConfirm_alreadyAttachs">
					...
					<c:otherwise>
						<tr id="yans_taskConfirm_attachSelect" style="display:none;">
							<th width="150px" >附件上传：</th>
							<td style="text-align:left" colspan=7>
								<div id="yans_taskConfirm_attachs"></div>
								<input type="file" id="yans_taskConfirm_attach" name="attach"/>
						...
						<tr id="yans_taskConfirm_attachSubmit" style="display:none;">
							<th width="150px" >已上传附件：</th>
							<td style="text-align:left" colspan=7>
								<div id="yans_taskConfirm_alreadyAttachs"></div>
							</td>
						</tr>
					</c:otherwise>
				</c:choose>
				...
				<c:choose>
				   	<c:when test="${taskOrder.ersaiFlag==1}">
						<tr id="yans_taskConfirm_ersaiNum">
					...
					<c:otherwise>
						<tr id="yans_taskConfirm_ersaiNum" style="display:none;">
							<th style="width: 150px">二筛任务单号</th>
							<td width=174 colspan=2>
								<input  name="ersaiLsh"
								style="width:330px" />
				...
				<tr>
			         <td colspan=8 style="text-align:center">
			         	<a id="yans_confirm_submit" class="l-btn" href="javascript:void(0);" onclick="javascript:confirmOKSubmit()">
			         		<span class="l-btn-left">
			         			<span class="l-btn-text icon-save l-btn-icon-left" style="padding-left: 20px; ">审核通过</span>
			         		</span>
			         	</a>
			         	<a id="yans_confirm_ng" class="l-btn" href="javascript:void(0);" onclick="javascript:confirmNGSubmit()">
			         		<span class="l-btn-left">
			         			<span class="l-btn-text icon-cancel l-btn-icon-left" style="padding-left: 20px; ">审核不通过</span>
			         		</span>
			         	</a>
			         	<a id="yans_confirm_close" class="l-btn" href="javascript:void(0);" onclick="javascript:closeCurrentTab()">
			         		<span class="l-btn-left">
