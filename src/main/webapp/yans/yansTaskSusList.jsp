...
<script type="text/javascript">
	$(function() {
		...
		$('#yans_taskorder_suslist_datagrid').datagrid({
			url : '${pageContext.request.contextPath}/yansTaskManage/susList',
			fit : true,
			fitColumns : false,
			border : false,
			pagination : true,
	...
	function yans_taskSusList_searchFun() {
		var startTime = $('#yansSus_registTimeStart').val();
		var endTime = $('#yansSus_registTimeEnd').val();
		if(comparetime(startTime,endTime)){
			$('#yans_taskorder_suslist_datagrid').datagrid('load', serializeObject($('#yans_taskSusList_searchForm')));
		}else{
			$.messager.show({
				title : '提示',
				msg : "开始时间大于结束时间"
			...
			if (r) {
				$.ajax({
					...
					success : function(result) {
						if (result.success) {
							$('#yans_taskorder_suslist_datagrid').datagrid('load');
						}
						$.messager.show({
							title : '提示',
							msg : result.msg
	...
	function yans_taskSusList_downloadFun() {
		var startTime = $('#yansSus_registTimeStart').val();
		var endTime = $('#yansSus_registTimeEnd').val();
		if(comparetime(startTime,endTime)){
			$('#yans_taskSusList_searchForm').attr('action','${pageContext.request.contextPath}/yansTaskManage/downloadExcel');
			$('#yans_taskSusList_searchForm').submit();
		}else{
			$.messager.show({
				title : '提示',
				msg : "开始时间大于结束时间"
	...
	function yans_taskSusList_cleanFun() {
		$('#yans_taskSusList_searchForm input').val('');
		$('#yans_taskorder_suslist_datagrid').datagrid('load', {});
	}
</script>
<div class="easyui-layout" data-options="fit : true,border : false">
	<div data-options="region:'north',border:false" style="height: 150px;overflow: hidden;" align="center">
		<form id="yans_taskSusList_searchForm" method="post" >
			<table align="center" class="tableFormcx" cellSpacing=0 cellPadding=5 style="margin-top: 10px;">
				...
				<tr>
					...
					<td><input id="yansSus_registTimeStart" name="registTimeStart" onFocus="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" /></td>
					<td style="width: 60px; text-align: right;">创建时间止</td>
					<td><input id="yansSus_registTimeEnd" name="registTimeEnd" onFocus="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" /></td>
				</tr>
				<tr>
					<td colspan="6" style="text-align: center;">
						<div>
							<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-search',plain:true" onclick="yans_taskSusList_searchFun();return false;">查询</a>&nbsp;
							<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-print',plain:true" onclick="yans_taskSusList_downloadFun();return false;">导出数据</a>&nbsp;
							<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-cancel',plain:true" onclick="yans_taskSusList_cleanFun();return false;">清空条件</a>
						</div>
					</td>
				</tr>
			</table>
	...
	<div data-options="region:'center',border:false">
		<table id="yans_taskorder_suslist_datagrid"></table>
	</div>
</div>