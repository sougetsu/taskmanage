...
<script type="text/javascript">
	$(function(){
		...
		var lastIndex;
		var editable=false;
		$('#yans_checkPriceEdit_datagrid').datagrid({
		    onClickRow:function(rowIndex){  
				if(editable==false){
					return false;
				}
		        if (lastIndex != rowIndex){  
		            $('#yans_checkPriceEdit_datagrid').datagrid('endEdit', lastIndex);  
		            $('#yans_checkPriceEdit_datagrid').datagrid('beginEdit', rowIndex);  
		            setEditing(rowIndex);  
		        }
		        lastIndex = rowIndex;  
		    },
	...
	function setEditing(rowIndex){ 
	    var editors = $('#yans_checkPriceEdit_datagrid').datagrid('getEditors', rowIndex);  
	    var rows = $('#yans_checkPriceEdit_datagrid').datagrid('getRows'); 
	...
	function endEdit(){
        ...
        for ( var i = 0; i < rows.length; i++) {
        	...
        	$('#yans_checkPriceEdit_datagrid').datagrid('endEdit', i); 
        }
	...
	function checkPriceSubmit(){
		...
		if(!comparetime(pakstartDate,pakendDate)){
			...
			return;
	            ...
	            var updated = $('#yans_checkPriceEdit_datagrid').datagrid('getChanges', "updated");
	            var effectRow = new Object();
	            if (updated.length) {
	           	 effectRow = obj2str(updated);
	           	 $("#yans_priceEditItems").val(effectRow);
	            }
	            var submitForm = $('#yans_taskOrder_priceEdit_form');
	            $.ajax({
	           	type: "POST",
...
<div class="easyui-layout" data-options="fit : true,border : false">
	<div data-options="region:'center',border:false" style="position:relative;left:0;top:0;">
		<form id="yans_taskOrder_priceEdit_form" method="post">
		<div align="center" style="width:780px;margin-left: auto;margin-right: auto;">
			...
			<input type="hidden" id="yans_priceEditItems" name="priceItems"/>
			<table id="yans_checkPriceEdit_datagrid" style="width:auto;height:auto"
				title="封测中心内部结算"
				singleSelect="true" idField="itemId" url="${pageContext.request.contextPath}/yansTaskManage/listPriceEdit?id=${taskOrder.orderId}">
				<thead>
					<tr>
			...
			<table>
				<tr>
					...
					<td width="80" align="right" id ="yans_sumPrice">${taskOrder.sumPrice}</td>
