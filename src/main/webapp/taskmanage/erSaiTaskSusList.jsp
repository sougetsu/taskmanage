...
<script type="text/javascript">
	$(function() {
		...
		$('#erSai_taskorder_suslist_datagrid').datagrid({
			url : '${pageContext.request.contextPath}/erSaiTaskManage/susList',
			fit : true,
			fitColumns : false,
			border : false,
			pagination : true,
	...
	function erSai_taskSusList_searchFun() {
		var startTime = $('#erSaiSus_registTimeStart').val();
		var endTime = $('#erSaiSus_registTimeEnd').val();
		if(comparetime(startTime,endTime)){
			$('#erSai_taskorder_suslist_datagrid').datagrid('load', serializeObject($('#erSai_taskSusList_searchForm')));
		}else{
			$.messager.show({
				title : '提示',
				msg : "开始时间大于结束时间"
			...
			if (r) {
				$.ajax({
					...
					success : function(result) {
						if (result.success) {
							$('#erSai_taskorder_suslist_datagrid').datagrid('load');
						}
						$.messager.show({
							title : '提示',
							msg : result.msg
	...
	function erSai_taskSusList_downloadFun() {
		var startTime = $('#erSaiSus_registTimeStart').val();
		var endTime = $('#erSaiSus_registTimeEnd').val();
		if(comparetime(startTime,endTime)){
			$('#erSai_taskSusList_searchForm').attr('action','${pageContext.request.contextPath}/erSaiTaskManage/downloadExcel');
			$('#erSai_taskSusList_searchForm').submit();
		}else{
			$.messager.show({
				title : '提示',
				msg : "开始时间大于结束时间"
	...
	function erSai_taskSusList_cleanFun() {
		$('#erSai_taskSusList_searchForm input').val('');
		$('#erSai_taskorder_suslist_datagrid').datagrid('load', {});
	}
</script>
<div class="easyui-layout" data-options="fit : true,border : false">
	<div data-options="region:'north',border:false" style="height: 150px;overflow: hidden;" align="center">
		<form id="erSai_taskSusList_searchForm" method="post" >
			<table align="center" class="tableFormcx" cellSpacing=0 cellPadding=5 style="margin-top: 10px;">
				...
				<tr>
					...
					<td><input id="erSaiSus_registTimeStart" name="registTimeStart" onFocus="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" /></td>
					<td style="width: 60px; text-align: right;">创建时间止</td>
					<td><input id="erSaiSus_registTimeEnd" name="registTimeEnd" onFocus="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd HH:mm:ss'})" /></td>
				</tr>
				<tr>
					<td colspan="6" style="text-align: center;">
						<div>
							<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-search',plain:true" onclick="erSai_taskSusList_searchFun();return false;">查询</a>&nbsp;
							<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-print',plain:true" onclick="erSai_taskSusList_downloadFun();return false;">导出数据</a>&nbsp;
							<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'icon-cancel',plain:true" onclick="erSai_taskSusList_cleanFun();return false;">清空条件</a>
						</div>
					</td>
				</tr>
			</table>
	...
	<div data-options="region:'center',border:false">
		<table id="erSai_taskorder_suslist_datagrid"></table>
	</div>
</div>