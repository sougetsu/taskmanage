...
<script type="text/javascript">
	$(function() {
		...
		$("#erSai_taskEdit_attach").uploadify({
			...
			queueID:"erSai_taskEdit_attachs",//上传队列说存储的位置
		...
		function fileComplete(event, ID, fileObj, response, data) {
			...
			if(checkOp(rel)) {
				...
				$("#erSai_taskEdit_alreadyAttachs").append(node);
			} else {
				alert(rel.err);
			}
		}
	...
	function editSubmit(){
		...
		if(submitForm.form('validate')){
    		...
    		$.ajax({
                type: "POST",
                url:'${pageContext.request.contextPath}/erSaiTaskManage/editSubmit',
...
<div data-options="region:'center',border:false">
	<div align="center">
		<form id="erSai_taskOrder_editForm"method="post">
			...
			<table class="tasktableForm" width="95%">
				...
				<c:choose>
				   	<c:when test="${taskOrder.attachmentFlag==1}">
						<tr id="erSai_taskEdit_attachSelect">
							<th width="150px" >附件上传：</th>
							<td style="text-align:left" colspan=7>
								<div id="erSai_taskEdit_attachs"></div>
								<input type="file" id="erSai_taskEdit_attach" name="attach"/>
								<br/>
								<a href="javascript:$('#erSai_taskEdit_attach').uploadifyUpload()">上传文件</a>
							</td>
						</tr>
						<tr id="erSai_taskEdit_attachSubmit">
							<th width="150px" >已上传附件：</th>
							<td style="text-align:left" colspan=7>
								<div id="erSai_taskEdit_alreadyAttachs">
					...
					<c:otherwise>
						<tr id="erSai_taskEdit_attachSelect" style="display:none;">
							<th width="150px" >附件上传：</th>
							<td style="text-align:left" colspan=7>
								<div id="erSai_taskEdit_attachs"></div>
								<input type="file" id="erSai_taskEdit_attach" name="attach"/>
						...
						<tr id="erSai_taskEdit_attachSubmit" style="display:none;">
							<th width="150px" >已上传附件：</th>
							<td style="text-align:left" colspan=7>
								<div id="erSai_taskEdit_alreadyAttachs"></div>
							</td>
						</tr>
					</c:otherwise>
				</c:choose>
				...
				<tr>
			         ...
			         <c:choose>
				   		<c:when test="${taskOrder.status==51 || taskOrder.status==52}">
