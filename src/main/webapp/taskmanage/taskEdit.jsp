...
<script type="text/javascript">
	...
	function setDetailRow(type,checktype){
		...
		if(type==3022){
			var taskpackage = document.getElementById("edit_task_mixpackage");
			taskpackage.style.display = "none";
			if(checktype){
				taskpackage.style.display = (document.all ? "block" : "table-row");
				$('#edit_mixpackage_table input').validatebox({ 
					required:true 
				});
			}else{
				$('#edit_mixpackage_table input').validatebox({ 
			...
			if(selectVal == 30)
			{
				$('#edit_mixpackage_table input[name="mdiscNum"]').validatebox({ 
					required:false
				});
			}
		}
		if(type==3023){
			var taskpackage = document.getElementById("edit_task_multipackage");
			taskpackage.style.display = "none";
			if(checktype){
				taskpackage.style.display = (document.all ? "block" : "table-row");
				$('#edit_multipackage_table input').validatebox({ 
					required:true 
				});
			}else{
				$('#edit_multipackage_table input').validatebox({ 
			...
			if(selectVal == 30)
			{
				$('#edit_multipackage_table input[name="mcdiscNum"]').validatebox({ 
					required:false
				});
			}
		}
		...
		<form id="taskOrder_editForm"method="post">
			...
			<table class="tasktableForm" width="95%">
				...
				<tr>
					...
					<td width=174 colspan=2>
						<input id="edit_electricId" name="electricId" class="easyui-combotree" style="width:330px;height:26px" data-options="url:'${pageContext.request.contextPath}/dictionary/electricList',parentField : 'pid'"
						lines="true" cascadeCheck="false" value="${taskOrder.electricId}"/>
					</td>
					<th width=121 colspan=2>项目名称</th>
					<td width=264 colspan=3>
						<input id="edit_projectName" name="projectName" readonly="true"  value="${taskOrder.projectName}" style="width:330px;font-size: 12px"/>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td width=174 colspan=2><input name="applyDept" class="easyui-combotree" style="width:330px;height:26px" data-options="url:'${pageContext.request.contextPath}/dictionary/organizationListText',parentField : 'pid',required:'true'"
						lines="true" cascadeCheck="false" value="${taskOrder.applyDept}"/></td>
				...
				<tr>
					...
					<td width=174 colspan=2>
						<input id="edit_topicNo" name="topicNo" readonly="true" style="width:330px;font-size: 12px" value="${taskOrder.topicNo}"/>
					</td>
				...
				<tr>
					...
					<td width=174px style="text-align:left" colspan=2>
					 	<span style="white-space:nowrap;">
							<label><input type="radio" style="width:30px" id="attachmentYes" name="attachmentFlag" onclick="setAttachRow(this.value)" value="1" ${taskOrder.attachmentFlag == 1 ? 'checked' : ''}/>是</label>
							<label><input type="radio" style="width:30px" id="attachmentNo" name="attachmentFlag" onclick="setAttachRow(this.value)" value="0" ${taskOrder.attachmentFlag == 0 ? 'checked' : ''}/>否</label>
						</span>
					</td>
					<td width=121 colspan=2>
						<span style="white-space:nowrap;">
						<b>监制：</b>
						<label><input type="radio" style="width:30px" id="superviseYes" name="superviseFlag" value="1" ${taskOrder.superviseFlag == 1 ? 'checked' : ''}/>是</label>
						<label><input type="radio" style="width:30px" id="superviseNo" name="superviseFlag" value="0" ${taskOrder.superviseFlag == 0 ? 'checked' : ''}/>否</label>
						</span>
					</td>
					<th >监制单位</th>
				...
				<c:choose>
					...
					<tr id="edit_task_mixpackage">
						<th width="150px"></th>
						<td width=558 colspan=7>
							<table id="edit_mixpackage_table" border="1" bordercolor="#B5C0C4" style="border-collapse:collapse;">
							   <tr >
							    <td width=31 rowspan=7 >
				   	...
				   	<c:otherwise>
				   		<tr id="edit_task_mixpackage" style="display:none;">
							<th width="150px"></th>
							<td width=558 colspan=7>
								<table id="edit_mixpackage_table" border="1" bordercolor="#B5C0C4" style="border-collapse:collapse;">
								   <tr >
				...
				<c:choose>
					...
					<tr id="edit_task_multipackage">
						<th width="150px"></th>
						<td width=558 colspan=7>
							<table id="edit_multipackage_table" border="1" bordercolor="#B5C0C4" style="border-collapse:collapse;">
							   <tr >
							    <td width=31 rowspan=7 >
				   	...
				   	<c:otherwise>
				   		<tr id="edit_task_multipackage" style="display:none;">
							<th width="150px"></th>
							<td width=558 colspan=7>
								<table id="edit_multipackage_table" border="1" bordercolor="#B5C0C4" style="border-collapse:collapse;">
								<tr >
