...
<script type="text/javascript">
	$(function(){
		...
		var lastIndex;
		var editable=false;
		$('#checkPrice_datagrid').datagrid({
		    onClickRow:function(rowIndex){  
				if(editable==false){
					return false;
				}
		        if (lastIndex != rowIndex){  
		            $('#checkPrice_datagrid').datagrid('endEdit', lastIndex);  
		            $('#checkPrice_datagrid').datagrid('beginEdit', rowIndex);  
		            setEditing(rowIndex);  
		        }
		        lastIndex = rowIndex;  
		    },
	...
	function setEditing(rowIndex){ 
	    var editors = $('#checkPrice_datagrid').datagrid('getEditors', rowIndex);  
	    var rows = $('#checkPrice_datagrid').datagrid('getRows'); 
	...
	function endEdit(){
        ...
        for ( var i = 0; i < rows.length; i++) {
        	...
        	$('#checkPrice_datagrid').datagrid('endEdit', i); 
        }
	...
	function checkPriceSubmit(){
		...
		if(!comparetime(jdstartDate,jdendDate)){
		 ...
		 if ($('#checkPrice_datagrid').datagrid('getChanges').length) {
	            var updated = $('#checkPrice_datagrid').datagrid('getChanges', "updated");
	            var effectRow = new Object();
	            if (updated.length) {
	           	 effectRow = obj2str(updated);
	           	 $("#priceItems").val(effectRow);
	            }
	            var submitForm = $('#taskOrder_pricecheck_form');
	            $.ajax({
	           	type: "POST",
				url : '${pageContext.request.contextPath}/taskManageTest/checkPrice',
...
<div class="easyui-layout" data-options="fit : true,border : false">
	<div data-options="region:'center',border:false" style="position:relative;left:0;top:0;">
		<form id="taskOrder_pricecheck_form" method="post">
		<div align="center" style="width:780px;margin-left: auto;margin-right: auto;">
			...
			<input type="hidden" id="priceItems" name="priceItems"/>
			<table id="checkPrice_datagrid" style="width:auto;height:auto"
				title="封测中心内部结算"
				singleSelect="true" idField="itemId" url="${pageContext.request.contextPath}/taskManageTest/listPrice">
				<thead>
					<tr>
			...
			<table>
				<tr>
					...
					<td width="80" align="right" id ="sumPrice">0</td>
