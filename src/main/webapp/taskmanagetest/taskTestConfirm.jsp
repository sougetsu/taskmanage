...
<script type="text/javascript">
	$(function() {
		...
		$("#confirm_attach").uploadify({
			...
			script: '${pageContext.request.contextPath}/taskManageTest/uploadFile',
		...
		function bindEvent() {
			$(".deleteAttach").click(function(){
				...
				$.post("${pageContext.request.contextPath}/taskManageTest/deleteAttach",{id:id},function(data){
					if(data.success) {
						$(div).remove();
					} else {
						alert(data.msg);
	...
	function confirmOKSubmit(){
		...
		if(submitForm.form('validate')){
    		...
    		$.ajax({
                type: "POST",
                url:'${pageContext.request.contextPath}/taskManageTest/confirmOK',
	...
	function confirmNGSubmit(){
		...
		if(submitForm.form('validate')){
    		...
    		$.ajax({
                type: "POST",
                url:'${pageContext.request.contextPath}/taskManageTest/confirmNG',
                data:submitForm.serialize(),
                dataType : 'json',
				success : function(result) {
					parent.$.messager.progress('close');
	...
	$("#ConfirmTest_electricId").combotree({
    	onSelect:function(node){
    		$("#ConfirmTest_topicNo").val(node.value);
    		$("#ConfirmTest_projectName").val(node.annotation);
    	}    
    });
</script>
		...
		<form id="taskOrder_confirmForm"method="post">
			...
			<table class="tasktableForm" width="95%">
				...
				<tr>
					...
					<td width=174 colspan=2>
						<input id="ConfirmTest_electricId" name="electricId" class="easyui-combotree" style="width:330px;height:26px" data-options="url:'${pageContext.request.contextPath}/dictionary/electricList',parentField : 'pid'"
						lines="true" cascadeCheck="false" value="${taskOrder.electricId}"/>
					</td>
					<th width=121 colspan=2>项目名称</th>
					<td width=264 colspan=3>
						<input id="ConfirmTest_projectName" name="projectName" readonly="true"  value="${taskOrder.projectName}" style="width:330px;font-size: 12px"/>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td width=174 colspan=2>
						<input id="ConfirmTest_topicNo" name="topicNo" readonly="true" style="width:330px;font-size: 12px" value="${taskOrder.topicNo}"/>
					</td>
							...
							<td style="text-align:left" colspan=7>
								<div id="xxwhAdd_alreadyAttachs">
									<c:forEach items="${taskOrder.attachment}" var="item">
										...
										<a href="${pageContext.request.contextPath}/taskManageTest/attached?filePath=${item.newName}">查看附件</a>
