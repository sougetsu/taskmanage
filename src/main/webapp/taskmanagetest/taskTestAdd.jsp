...
<script type="text/javascript">
	...
	function setDetailRow(type,checktype){
		...
		if(type==11){
			...
			if(checktype){
				taskpackage.style.display = (document.all ? "block" : "table-row");
				$('#createTest_package_table input').validatebox({ 
					required:true 
				});
			}else{
				$('#createTest_package_table input').validatebox({ 
			...
			if(selectVal == 30)
			{
				$('#createTest_package_table input[name="discNum"]').validatebox({ 
					required:false
				});
			}
		}
	...
	function checkAndSubmit(){
		var submitForm = $('#taskOrderTest_createForm');
		if(submitForm.form('validate')){
    		...
    		$.ajax({
                type: "POST",
                url:'${pageContext.request.contextPath}/taskManageTest/create',
...
<div data-options="region:'center',border:false">
	<div align="center">
		<form id="taskOrderTest_createForm"method="post" enctype="multipart/form-data">
		<input type="hidden" id="status" name="status">
			<table class="tasktableForm" width="95%" border="1"  bordercolor="#B5C0C4" rules="none" style="border-collapse:collapse;">
				...
				<tr id="task_package" style="display:none;">
					<th width="150px"></th>
					<td width=558 colspan=7>
						<table id="createTest_package_table" border="1" bordercolor="#B5C0C4" style="border-collapse:collapse;">
						   <tr >
